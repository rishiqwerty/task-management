...
from tasks.prompt import (
    VALID_PRIORITIES,
    generate_task_from_text_prompt,
    generate_task_prioritization_prompt,
    gemini_api_call,
    openai_api_call,
)
...
def assign_task_priority(task_id):
    ...
    try:
        current_task = (
            ...
            .values_list("id", "title", "due_date", "description", "priority", "tag")
            .first()
        )
        task = (
            Task.objects.filter(
                due_date__gte=current_task[2], status__in=[Task.TaskStatus.PENDING]
            )
            .values_list("id", "title", "due_date", "description", "priority", "tag")
            .order_by("-due_date")[:10]
        )
        prompt = (
            f"""You are a task prioritization assistant.
        ...
        p.save()
        print(
            f"Task ID {task_id} priority assigned successfully: {priority}, tag: {tag}"
        )
    except Exception as e:
        status["error"] = str(e)
...
def generate_task_from_text(text):
    ...
    try:
        ...
        for task in task_data:
            due_date = (
                datetime.strptime(task.get("due_date"), "%Y-%m-%dT%H:%M:%SZ")
